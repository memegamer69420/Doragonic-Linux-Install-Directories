...
mkdir -p doragonic-binds
bin=start-doragonic.sh
...
command+=" -0"
command+=" -r $folder"
if [ -n "\$(ls -A doragonic-binds)" ]; then
    for f in doragonic-binds/* ;do
      . \$f
    done
fi
command+=" -b /dev"
command+=" -b /proc"
command+=" -b doragonic-fs/root:/dev/shm"
...
echo "Setting up pulseaudio so you can have music in Doragonic."
...
rm $tarball
echo "You can now launch Doragonic Linux 1.0 with the ./${bin} script"
